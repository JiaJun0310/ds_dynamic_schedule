...
app.post("/sendData", verifyToken, async (req, res) => {
    try {
        const springPath = path.join(
            __dirname,
            "jsonData",
            "spring_schedule.json",
        );
        const winterPath = path.join(
            __dirname,
            "jsonData",
            "winter_schedule.json",
        );
        const mergedPath = path.join(
            __dirname,
            "jsonData",
            "merged_schedule.json",
        );
...
app.post("/sendLabData", verifyToken, async (req, res) => {
    try {
        ...
        const springData = fs.existsSync(springPath)
            ? JSON.parse(fs.readFileSync(springPath, "utf8"))
            : [];
        const winterData = fs.existsSync(winterPath)
            ? JSON.parse(fs.readFileSync(winterPath, "utf8"))
            : [];
...
app.post("/sendExamData", verifyToken, async (req, res) => {
    try {
        ...
        const makeUpData = fs.existsSync(makeUpPath)
            ? JSON.parse(fs.readFileSync(makeUpPath, "utf8"))
            : [];
        const semesterData = fs.existsSync(semesterPath)
            ? JSON.parse(fs.readFileSync(semesterPath, "utf8"))
            : [];
...
app.post("/getLabs", async (req, res) => {
    try {
        ...
        const filteredLabs = data.filter(
            (lab) => String(lab.semester) === String(semester),
        );
...
app.post("/getSearch", async (req, res) => {
    try {
        ...
        if (mode === "Μαθήματα") {
            ...
            titles = data
                .filter((course) =>
                    String(course.title).includes(String(search).toUpperCase()),
                ) // Filtering so we only return the results that match the search. We use "includes"
                .map((course) => ({ title: course.title }));
            ...
            titles = data
                .filter((exam) =>
                    String(exam.title).includes(String(search).toUpperCase()),
                ) // Filtering so we only return the results that match the search. We use "includes"
            ...
            titles = data
                .filter((lab) =>
                    String(lab.name).includes(String(search).toUpperCase()),
                ) // Filtering so we only return the results that match the search. We use "includes"
        ...
        if (data) {
            ...
            const filtered = data.filter((item) => {
                ...
                if (mode === "Μαθήματα") {
                    ...
                    if (teacher != "Διδάσκων") {
                        matchTeacher =
                            item.professor && item.professor.includes(teacher);
                    }

                    //Check if the room is in the room array of the item
                    if (room != "Αίθουσα") {
                        matchRoom =
                            item.lectureHall && item.lectureHall.includes(room);
                    }
                } else if (mode === "Εξεταστική") {
                    //Check if the room is in the room array of the item
                    if (room != "Αίθουσα") {
                    ...
                    if (room) {
                        matchRoom =
                            item.data &&
                            item.data.some((d) => d.labhall === room);
                    }
                }
...
app.post("/getExam", async (req, res) => {
    try {
        ...
        const examData =
            data.find((exam) => exam.title === title) ||
...
app.post("/getSemesterExams", async (req, res) => {
    try {
        ...
        const semesterExams = data.filter(
            (exam) => String(exam.semester) === String(semester),
        );
...
app.post("/getSemesterOfExams", async (req, res) => {
    try {
        ...
        if (fs.existsSync(makeUpPath)) {
            ...
            // Check if valid data exists
            if (
                Array.isArray(makeUpExams) &&
                makeUpExams.length > 0 &&
                makeUpExams[0].semester
            ) {
        ...
        return res
            .status(404)
            .json({ error: "No valid exams found in either file" });
    } catch (error) {
...
app.post("/login", async (req, res) => {
    try {
        ...
        if (incomingUsername !== username) {
            //validates username with hardcoded username
        ...
        const isPasswordCorrect = await bcrypt.compare(
            password,
            hashedPassword,
        ); //bcrypts compares the hash of the password with the hardcoded hash
        ...
        const token = jwt.sign(
            //creates a token that expires in 1 hour (for security)
...
app.post("/updateLab", async (req, res) => {
    try {
        const { name, semester, data } = req.body;
...
app.get("/getAcademicCalendar", (req, res) => {
    try {
        ...
        if (!fs.existsSync(filePath)) {
            return res
                .status(404)
                .json({ message: "Academic calendar file not found" });
        }
...
app.post("/updateAcademicCalendar", (req, res) => {
    try {
        ...
        if (
            ...
            return res
                .status(400)
                .json({ message: "Invalid academic calendar format" });
        }
